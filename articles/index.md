# Articles

### Getting Started

Start here for a quick overview of the rblp package, covering logit
estimation, random coefficients, simulation, and merger analysis.

- [Getting Started with
  rblp](https://hchulkim.github.io/rblp/articles/rblp-tutorial.md):
- [Preparing Data for
  rblp](https://hchulkim.github.io/rblp/articles/data-preparation.md):

### Model Estimation

Detailed guides on estimating demand models, from the plain logit to the
full random coefficients BLP specification with demographics.

- [Logit and Nested Logit
  Models](https://hchulkim.github.io/rblp/articles/logit-nested-logit.md):
- [Random Coefficients
  Logit](https://hchulkim.github.io/rblp/articles/random-coefficients.md):

### Post-Estimation & Policy Analysis

Use estimated demand parameters to compute elasticities, consumer
surplus, markups, diversion ratios, and simulate the effects of mergers.

- [Post-Estimation and Merger
  Simulation](https://hchulkim.github.io/rblp/articles/post-estimation-mergers.md):
- [Advanced Features: Optimal Instruments, Bootstrap, and
  More](https://hchulkim.github.io/rblp/articles/advanced-features.md):

### Tutorials & Exercises

Hands-on tutorials for learning BLP estimation, including Mixtape
Sessions exercises and Monte Carlo simulation studies.

- [Mixtape Demand Estimation
  Exercises](https://hchulkim.github.io/rblp/articles/mixtape-exercises.md):
- [Simulation and Monte
  Carlo](https://hchulkim.github.io/rblp/articles/simulation-monte-carlo.md):
- [Replicating baby_BLP: OTC Drug Demand
  Estimation](https://hchulkim.github.io/rblp/articles/baby-blp-replication.md):
- [Monte Carlo Validation of
  rblp](https://hchulkim.github.io/rblp/articles/monte-carlo-validation.md):
